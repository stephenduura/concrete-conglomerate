...
export default async function handler(req: any, res: any) {
  ...
  if (!host || !user || !pass) {
    ...
    return res.status(200).json({
      success: true,
      message: "Inquiry processed successfully (Simulated Dev Mode - Missing Credentials)",
    });
  }
  ...
  try {
    const transporter = nodemailer.createTransport({
      ...
      tls: {
        // Essential setting for certain hosting providers like Hostinger 
        // to bypass strict SSL verification issues in serverless Node runtimes
    ...
    // Bubble up the actual error message so the user can see details in their console
